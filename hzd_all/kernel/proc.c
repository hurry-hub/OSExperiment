...
{
	struct proc*	p;
	int queue_ticks[3] = {QUEUE_ONE_TIME, QUEUE_TWO_TIME, QUEUE_THREE_TIME};
	for (p = &FIRST_PROC; p <= &LAST_PROC; p++) {
		if (p->p_flags == 0 && p->arrive == 0) {
			p->ticks = queue_ticks[0] * TIME_PIECE;
			p->arrive = 1;
			p->status = READY;
			proc_queue[0][queue_tail[0]] = p;
			queue_tail[0] = (queue_tail[0] + 1) % (NR_TASKS + NR_PROCS + 1);
		}
	}
	for (int i = 0; i <= 2; i++) {
		while (1) {
			if (queue_head[i] == queue_tail[i]) {
				break;
			}
			p = proc_queue[i][queue_head[i]];
			if (p->p_flags == 0) {
				if (p->ticks) {
					p->status = RUN;
					p_proc_ready = p;
					...
					int a = (i == 2) ? 2 : i + 1;
					p->ticks = queue_ticks[a] * TIME_PIECE;
					p->status = READY;
					int tail = queue_tail[a];

					proc_queue[a][tail] = p;
					tail = (tail + 1) % (NR_TASKS + NR_PROCS + 1);
					queue_tail[a] = tail;
					queue_head[i] = (queue_head[i] + 1) % (NR_TASKS + NR_PROCS + 1);
				}
			} else {
				p->arrive = 0;
				p->status = WAIT;
