...
void manstep() //浜鸿蛋妫嬪鐞嗘ā鍧?
{
	int j;
	display(arr);
	if (arrfull()) //濡傛灉妫嬬洏涓婅繕鏈変笅妫嬬殑浣嶇疆锛岀粰浜鸿蛋涓€姝ユ
	{
...
void computerstep() //璁＄畻鏈鸿蛋妫嬪鐞嗘ā鍧?
{
	int i;
	if (arrfull()) //濡傛灉妫嬬洏涓婅繕鏈夊彲涓嬫鐨勪綅缃紝鍒欒绠楁満璧版
	{
		for (i = 1; i <= 9; i++) //瀵规瘡涓€姝ュ彲璧扮殑妫嬭繘琛岃绠?
		{
			if (i == arr[i] - 48)
			{
				c3 = 0; n2 = 0; c2 = 0; n1 = 0; c1 = 0;
				...
				number = linenum(arr[1], arr[5], arr[9]); cn(number);
				number = linenum(arr[3], arr[5], arr[7]); cn(number);
				brr[i] = (128 * c3 - 63 * n2 + 31 * c2 - 15 * n1 + 7 * c1); //璁＄畻姝ゆ鏉冨€?
				arr[i] = i + 48;
			}
			else
				brr[i] = -999;
		}
		arr[maxbrr(brr)] = 'X'; //纭畾璁＄畻鏈鸿蛋鍝竴姝ワ紝鏉冨€兼渶澶х殑涓€姝?
		c3 = 0; n2 = 0; c2 = 0; n1 = 0; c1 = 0;
		number = linenum(arr[1], arr[2], arr[3]); cn(number);
		number = linenum(arr[4], arr[5], arr[6]); cn(number);
		number = linenum(arr[7], arr[8], arr[9]); cn(number);
...
void ic()
{
	...
	printf("              *                                                 *\n");
	printf("              ***************************************************\n\n");
	inarrdata(arr); //妫嬬洏鍧愭爣缂栧彿
	display(arr); //鏄剧ず鍒濆妫嬬洏
	suc = 1;
	printf("Do you want to go first?(y/n)");
	char bufr[128];
	read(0, bufr, 128);
	if ((bufr[0] == 'y') || (bufr[0] == 'Y')) //杈撳叆Y锛岃〃绀轰汉鍏堣蛋妫?
	{
		while (suc)
		{
			manstep();
